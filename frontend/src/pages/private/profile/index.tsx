...
function ProfilePage() {
  ...
  return (
    <div className="dark:bg-gray-900 bg-white min-h-screen pb-20">
      {/* Profile Gallery */}
      <div className="relative h-[70vh] max-h-[700px] overflow-hidden">
        {userInfo?.images?.length > 0 ? (
          ...
          <div className="h-full w-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
            <FaRegUser className="text-9xl text-gray-300 dark:text-gray-600" />
          </div>
        )}
        ...
        <div className="absolute bottom-20 left-0 right-0 flex justify-center gap-4 ">
          ...
          <motion.button
            ...
            className={`p-4 rounded-full shadow-lg ${
              liked ? "bg-red-primary text-white" : "bg-white text-red-primary"
            }`}
          >
            {liked ? (
      ...
      <motion.div
        ...
        className="relative z-10 mx-4 -mt-16 bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6"
      >
        ...
        {userInfo?.bio && (
          <div className="mt-4">
            <p className="text-gray-700 dark:text-gray-300">{userInfo.bio}</p>
          </div>
        )}
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {userInterests.map((interest: any) => (
                <span
                  key={interest.id}
                  className="bg-red-primary/10 text-red-primary px-3 py-1 rounded-full text-sm"
                >
                  #{interest.name}
                </span>
              ))}
            ...
            {userInfo.images.map((_, index) => (
              <div
                ...
                className={`h-1.5 rounded-full ${
                  index === 0 ? "w-6 bg-red-primary" : "w-2 bg-gray-300"
                }`}
              />
            ))}
          </div>
